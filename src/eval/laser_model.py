...
class TransformersTextEmbedder(torch.nn.Module):
    def __init__(
        ...
        **kwargs,
        ...
        super().__init__()
        self.add_chat_template = kwargs.pop('add_chat_template', False)
    ...
    def forward(
        ...
        **kwargs
        ...
        if self.num_thinking_steps > 0:
            attention_mask = torch.cat([
                attention_mask, 
                torch.ones(batch_size, self.num_thinking_steps, dtype=torch.long, device=input_ids.device)
            ], dim=1)
        
        original_input_embeddings = self.base_model.get_input_embeddings()(input_ids)
        
        collected_last_hidden = []
        past_key_values = None
        next_input_embeds = original_input_embeddings
        embedding_table = self.base_model.get_input_embeddings().weight

        for step_idx in range(self.num_thinking_steps-1):
            current_seq_len = original_input_embeddings.shape[1] + step_idx
            input_attention_mask = attention_mask[:, :current_seq_len]
            ...
            collected_last_hidden.append(last_hidden_state)

            last_token_logits = outputs.logits[:, -1, :]
            weights = F.softmax(last_token_logits, dim=-1)
            new_thought_embedding = torch.matmul(weights, embedding_table)
            next_input_embeds = new_thought_embedding.unsqueeze(1)
        
        final_seq_len = original_input_embeddings.shape[1] + self.num_thinking_steps
        final_attention_mask = attention_mask[:, :final_seq_len]

        if past_key_values is None:
            outputs = self.base_model(
                inputs_embeds=original_input_embeddings,
        ...
        last_hidden_state = get_thought_embedding_v1(outputs)
        collected_last_hidden.append(last_hidden_state)
        
        embeddings = torch.stack(collected_last_hidden, dim=1) 
...
class LaserModel(Wrapper):
    ...
    def __init__(
        ...
        **kwargs,  # For `TransformersTextEmbedder`
        ...
        self.model = self._model_class(model, num_thinking_steps=num_thinking_steps, **kwargs)
