...
class EvalArguments:
    ...
    num_thinking_steps: int = field(default=3, metadata={"help": "number of thinking steps"})
...
def get_model(model_path: str, model_name: str, precision: str = 'fp16', num_thinking_steps: int = 0, **kwargs):
    model = LaserModel(model_path, model_name=model_name, precision=precision, **kwargs)
    return model
...
def main():
    ...
    if args.only_load:
        ...
        if not args.load_model:
            return
    model = get_model(args.model, precision=args.precision, num_thinking_steps=args.num_thinking_steps, **args.model_kwargs)
    if args.only_load:
        return
