...
@dataclass
class TrainCollatorWithRewrite:
    """
    ...
    def _add_chat_template(self, text):
    ...
    def __call__(self, features: List[Tuple[str, List[str]]]):
        ...
        all_rewrite_query = [f[2] for f in features]
        all_passages = []
        for f in features:
            all_passages.extend(f[1])

        if self.data_args.add_chat_template:
            all_queries = [self._add_chat_template(q) for q in all_queries]
            all_passages = [self._add_chat_template(p) for p in all_passages]
        
        all_rewrite_query = [query + rewrite for query, rewrite in zip(all_queries, all_rewrite_query)]
        ...
        if self.data_args.append_eos_token:
            ...
            r_collated['input_ids'] = [r + [self.tokenizer.eos_token_id] for r in r_collated['input_ids']]
        ...
        return (q_collated, r_collated), d_collated
