...
def _format_query_with_prompt(template: str, prompt: str, query: str) -> str:
    """Format query text with prompt template while keeping backward compatibility."""
    if not template:
        return query

    if "{query}" in template or "{instruct}" in template:
        return template.format(instruct=prompt, query=query)

    if "{}" in template:
        return template.format(prompt) + query
...
class TrainDataset(Dataset):
    ...
    def __getitem__(self, item):
        ...
        formatted_query = _format_query_with_prompt(
        ...
        positive_text = selected_positive
        ...
        if len(group['negative_passages']) < negative_size:
            ...
            selected_negatives = group['negative_passages'][:negative_size]
...
class TrainDatasetWithRewrite(Dataset):
    """
    Dataset for training which handles both query and passage data.
    Loads dataset and optional corpus from the provided paths/configurations.
    """
    ...
    def __getitem__(self, item):
        ...
        # Handling the legacy format with 'positive_passages'
        query_text = group['query']
        rewrite_query = group['reasoning_query']
        # 避免没有rewrite query
        if rewrite_query == '' or len(rewrite_query) < 5:
        ...
        return formatted_query, formatted_documents, rewrite_query
